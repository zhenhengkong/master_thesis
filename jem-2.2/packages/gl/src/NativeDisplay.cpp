...
void NativeDisplay::Command_::exec ( Display& disp )
{
  ...
  {
    if ( win_ != NIL && win_->errHandler != NIL )
    {
...
{
  JEM_ASSERT ( win     != NIL && output != NIL &&
               grabber != NIL && scene  != NIL );
}
...
void NativeDisplay::GrabFrameCmd_::run ( Display& disp )
{
  ...
  disp.takeAction_   ( *win_, *scene_, GLActions::GRAB_FRAME );
...
const double    NativeDisplay::MIN_LOD_      = 0.01;

const int       NativeDisplay::INITIALIZED_  = 1 << 0;
const int       NativeDisplay::IN_MAIN_LOOP_ = 1 << 1;

NativeDisplay*  NativeDisplay::instance_     = 0;
Thread*         NativeDisplay::thread_       = 0;
...
NativeDisplay::NativeDisplay ()
{
  ...
  // Two pre-allocated Integer objects are used to pass the mouse/key
  // position to the scene graph. This is more efficient than adding
  // two integers to a Properties object each time an event is
  // handled.
...

{
  ...
  if ( lod_ < 0.0 )
  {
    ...
    takeAction_ ( win, scene, GLActions::REDRAW_FAST );
    ::glFlush   ();

    lod_ *= DELTA / (timer_.toDouble() + 0.001);

    clamp ( lod_, MIN_LOD_, 1.0 );
  ...
  newLod = lod_ * DELTA / (timer_.toDouble() + 0.001);

  if ( avgLod_ < 0.0 )
  {
    avgLod_ = 0.5 * (newLod + lod_);
    lod_    = clamped ( avgLod_, MIN_LOD_, 1.0 );
  }
  else
  {
  ...
  while ( events_.size() )
  {
    ...
    case GfxEvent::REDRAW:

      {
        ...
        else
        {
          timer_.reset ();
          timer_.start ();
          takeAction_  ( win, *rsc, GLActions::REDRAW );
          ::glFlush    ();
          timer_.stop  ();
    ...
    case GfxEvent::RESIZE:

      {
        ...
        takeAction_ ( win, scene, GLActions::INIT_VIEW );
      }
    ...
    case GfxEvent::INIT:
      ...
      takeAction_ ( win, scene, GLActions::INIT_GL );
    ...
    case GfxEvent::MOUSE:

      {
        ...
        takeAction_ ( win, scene, GLActions::MOUSE_EVENT );
      }
    ...
    case GfxEvent::MOTION:

      {
        ...
        takeAction_ ( win, scene, GLActions::MOTION_EVENT );
      }
    ...
    case GfxEvent::CLICK2:

      {
        ...
        takeAction_ ( win, scene, GLActions::CLICK2_EVENT );
      }
    ...
    case GfxEvent::KEY:

      {
        ...
        takeAction_ ( win, scene, GLActions::KEY_EVENT );
      }
    ...
    case GfxEvent::COMMAND:

      {
        ...
        takeAction_ ( win, scene, GLActions::EXE_COMMAND );
      }
...
{
  gtx.postRedraw ( staticCast<GfxNode>( obj ) );
}
