...
class GlutWindow : public NativeWindow
{
 public:
  ...
  virtual Size2i            getWindowSize   () const;
...
void GlutDisplay::keyFunc_ ( unsigned char key, int x, int y )
{
  y = glutGet( GLUT_WINDOW_HEIGHT ) - y;
  ...
  try
  {
    ...
    if ( (mods & Keys::CTRL) && key >= 1 && key <= 26 )
    {
      ...
      key = (unsigned char) ('a' + key - 1);
    }

    instance_->postEvent_ (
      glutGetWindow (),
      new KeyEvent  ( (int) key, mods, x, y )
    );
  }
  catch ( ... ) {}
}
