...

{
  ...
  if ( elems == NIL || elems.getNodes() != nodes )
  {
...

{
  ...
  else
  {
    ...
    // Note that the shape table may be NIL if the rank of the mesh
    // is not supported.
...

{
  ...
  if ( rundat_ != NIL && rundat_->shapes != NIL )
  {
    Properties  globdat = viewdat.getProps ( Viewdat::GLOBDAT );
