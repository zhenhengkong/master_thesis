...

{
  ...
  k  = (ulint) i[0] + JEM_ULINT_C(0x9e3779b9);
  k ^= (ulint) i[1] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
...

{
  ...
  k  = (ulint) i[0] + JEM_ULINT_C(0x9e3779b9);
  k ^= (ulint) i[1] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
  k ^= (ulint) i[2] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
...

{
  ...
  k ^= (ulint) i[1] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
  k ^= (ulint) i[2] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
  k ^= (ulint) i[3] + JEM_ULINT_C(0x9e3779b9) + (k << 6) + (k >> 2);
...
template <class T, int N, class A>
  ...
  inline typename SparseArray<T,N,A>::Iterator
  SparseArray<T,N,A>::end ()

{
  const ulint  n = JEM_ULINT_C(1) << tableBits_;
...
template <class T, int N, class A>
  ...
  void SparseArray<T,N,A>::swap ( SparseArray& rhs )
...
template <class T, int N, class A>
  ...
  inline idx_t SparseArray<T,N,A>::capacity () const
...
template <class T, int N, class A>
  ...
  inline idx_t SparseArray<T,N,A>::size () const
...
template <class T, int N, class A>
  ...
  inline const T& SparseArray<T,N,A>::getZero () const
...
template <class T, int N, class A>
  ...
  inline idx_t SparseArray<T,N,A>::nonZeroCount () const
...

{
  ...
  tableMask_  = (JEM_ULINT_C(1) << tableBits_) - JEM_ULINT_C(1);
  nodeCount_  = 0;
  capacity_   = cap;
  zero_       = new T ( zero );

  n      = JEM_ULINT_C(1) << tableBits_;
  table_ = (void**) MemCache::alloc ( sizeof(*table_) * (n + 1) );
...

{
  ...
  {
    clear ();
    MemCache::dealloc ( table_, sizeof(*table_) * (n + 1) );
    table_ = NULL;
  }
...

{
  ...
  const ulint  n = JEM_ULINT_C(1) << rhs.tableBits_;
...

{
  ...
  oldSize  = JEM_ULINT_C(1) << tableBits_;
  newBits  = sparse::getTableBits ( (ulint) cap );
  newSize  = JEM_ULINT_C(1) << newBits;
  newMask  = newSize - JEM_ULINT_C(1);
  newTable = (void**) MemCache::alloc ( sizeof(void*) *
                                        (newSize + 1) );
  ...
  MemCache::dealloc ( oldTable, sizeof(*oldTable) * (oldSize + 1) );
}
