...
{
  JEM_PRECHECK ( order >= 1 && order <= 2 && model != NIL );
  ...
  int         traits = SparseMatrixObj::SYMMETRIC;
  bool        sparse = true;
...

{
  ...
  for ( int i = 0; i < 3; i++ )
  {
    if ( matrices_[i] == NIL || isTiny( coeffs_[i] ) )
    {
      continue;
    }
...
bool TransientMatrix::hasTrait ( const String& trait ) const
{
  ...
  for ( int i = 0; i < 3; i++ )
  {
    if ( matrices_[i] != NIL && ! matrices_[i]->hasTrait( trait ) )
    {
      result = false;
    }
  }
...

{
  ...
  if ( ! (updated_ & U_MATRIX_) && transmat_ != NIL )
  {
    const Shape  sh = matrices_[0]->shape ();
