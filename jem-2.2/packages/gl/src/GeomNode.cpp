...

{
  ...
  if ( attribs_ != NIL )
  {
    if ( newFields_ )
    {
      ...
      passAction       ( child_, Actions::SET_ATTRIBS, props );
  ...
  bool  result = passGLAction ( child_, gtx, action, params );
...
Attributes* GeomNode::getAttribs () const
{
  return attribs_.get ();
}
...
void GeomNode::newAttribs_ ( int fields )
{
  newFields_ |= fields;
}
