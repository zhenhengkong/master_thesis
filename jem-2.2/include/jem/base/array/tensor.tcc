...

{
  ...
  stride_  = a.stride ();
  value_   = JEM_IDX_C(0);
  stride0_ = (ix == addr_) ? stride_ : JEM_IDX_C(0);
  stride1_ = (jx == addr_) ? stride_ : JEM_IDX_C(0);
}
...
{
  T  t = op ( ax.getFast( JEM_IDX_C(0) ),
              ax.getFast( JEM_IDX_C(1) ) );

  for ( idx_t i = JEM_IDX_C(2); i < n; i++ )
  {
    t = op ( t, ax.getFast( i ) );
  }
...

{
  ...
  if ( select( index_, ix, ubound_, JEM_IDX_C(-1) ) >= 0 )
  {
    usedIndexError ();
  }
}
...
{
  return tensor::select ( index_, i, ubound_, JEM_IDX_C(-1) );
}
...

{
  ...
  *ix = JEM_IDX_C(0);
  ...
  if      ( stride_[0] == JEM_IDX_C(1) && ax.isContiguous() )
  {
    for ( idx_t i = 0; i < n; i++ )
    {
  ...
  else if ( n > JEM_IDX_C(0) )
  {
    const idx_t  st = stride_[0];

    p[JEM_IDX_C(0)] = op ( p [JEM_IDX_C(0)],
                            ax[JEM_IDX_C(0)] );
  ...
  do
  {
    ...
    else
    {
      ...
      for ( idx_t j = 0; j < n; j++ )
      {
        ...
        ax.advance ( ix, jx, j + JEM_IDX_C(1) );
      }
    }
  }
...

{
  ...
  {
    JEM_ASSERT2 ( i.first() >= JEM_IDX_C(0)  &&
                  i.first() <= i.last( size ) &&
                  i.stride() > JEM_IDX_C(0),
