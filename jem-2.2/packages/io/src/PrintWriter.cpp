...
const int  PrintWriter::FLUSH_LINES   = 1 << 0;
const int  PrintWriter::NO_WORD_WRAP  = 1 << 1;
const int  PrintWriter::NUMERIC_BOOLS = 1 << 2;
...
void PrintWriter::writeNoThrow ( const char* buf, idx_t n )
{
...
void PrintWriter::printBlankLine ()
{
  ...
  while ( d.blankLines < JEM_LINT_C(1) )
  {
    d.newLine ();
  }
}
...
void PrintWriter::setOptions ( int options )
{
...
int PrintWriter::getOptions () const
{
...
int PrintWriter::getPageWidth () const
{
...
int PrintWriter::getIndentWidth () const
{
...
int PrintWriter::getIndentLevel () const
{
...
int PrintWriter::getMarginWidth () const
{
  return data_->margin;
}
...
lint PrintWriter::getLineNumber () const
{
  return data_->lineno;
}
...
lint PrintWriter::getColumnNumber () const
{
  const Data_&  d = * data_;
...
bool PrintWriter::atStartOfLine () const
{
  const Data_&  d = * data_;
