...
const int  Properties::PARSE_STRICT   = 1 << 0;
const int  Properties::PARSE_TO_UPPER = 1 << 1;
const int  Properties::PARSE_INCLUDE  = 1 << 2;

const int  Properties::CLONE_STRICT   = 1 << 0;

const int  Properties::MERGE_REPLACE  = 1 << 0;
...
Properties Properties::clone ( int options ) const
{
  if ( dict_ == NIL )
  {
    return Properties ( NIL, conv_, myName_ );
  }
...
{
  JEM_PRECHECK2 ( dict != NIL, "NULL Dictionary" );
...
{
  if ( rhs.dict_ != this->dict_ && rhs.dict_ != NIL )
  {
...
#define JEM_DEFINE_SET_PROPERTY( Type )                 \
                                                        \
void Properties::set ( const String&  name,             \
                       Type           value ) const     \
{                                                       \
  set ( name, toObject( value ) );                      \
}
...
{
  set ( name, toObject( String( value ) ) );
}
...

{
  ...
  for ( idx_t i = 0; i < n; i++ )
  {
    if ( s[i] == '.' )
    {
      setRecursive_ ( name, i, value );

      return;
    }
  }

  dict_->insert ( name, value );
}
...

{
  ...
  {
    setRecursive_ ( name, idx, values.dict_ );
  }
}
...

{
  ...
  while ( last >= 0 )
  {
    if ( last > first )
    {
      ...
      if ( ! found || ! dynamicCast<Dict*>( obj ) )
      {
        ...
        props->insert ( name[slice(first,last)], obj );
      }
  ...
  props->insert ( name[slice(first,END)], value );
}
