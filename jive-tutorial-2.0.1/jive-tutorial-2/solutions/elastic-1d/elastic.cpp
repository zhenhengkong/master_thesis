...
// Sets the essential boundary conditions.
...
{
  idx_t  jtype = dofs.getTypeIndex ( DOF_TYPE );
  idx_t  idof  = dofs.getDofIndex  ( 0, jtype );
...
void                      solve
  ...
  ( Vector&                 lhs,     // solution vector
    ...
    Ref<Constraints>        cons )   // constraints

{
  ...
  Ref<Solver>  solver =
    ...
    newInstance<SparseLU> ( "solver", mbld.getMatrix(), cons );
...
int run ()
{
  ...
  System::out() << "Initializing the constraints ...\n";
