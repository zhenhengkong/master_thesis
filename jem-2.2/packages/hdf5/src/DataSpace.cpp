...
DataSpace::DataSpace ( const Handle& sid ) :
...

{
  if ( sid.isValid() )
  {
    rank_ = H5Sget_simple_extent_ndims ( (hid_t) sid.get() );
...

{
  ...
  else
  {
    ...
    else
    {
      ...
      sid = H5Screate_simple ( rank, dims.addr(), NULL );
    }
  ...
  sid_  = Handle ( sid, Handle::ALLOCATED );
  rank_ = rank;
}
...
DataSpace DataSpace::clone () const
{
  if ( sid_.isValid() )
  {
    hid_t  id = H5Scopy ( (hid_t) sid_.get() );
    ...
    return DataSpace (
      Handle ( (lint) id, Handle::ALLOCATED ),
      rank_
    );
  }
  else
...
idx_t DataSpace::itemCount () const
{
  ...
  hssize_t  count = H5Sget_select_npoints ( (hid_t) sid_.get() );
...
const DataSpace& DataSpace::selectAll () const
{
  ...
  if ( H5Sselect_all( (hid_t) sid_.get() ) < 0 )
  {
    throw H5Exception (
      JEM_FUNC,
  ...
  if ( rank >= 0 )
  {
    ...
    else
    {
      ...
      status = H5Sselect_hyperslab (
        (hid_t) sid_.get (),
        H5S_SELECT_SET,
        hoffset.addr(), hstride.addr(), hcount.addr(), NULL
      );
    }
...

{
  ...
  if ( stride > JEM_IDX_C(1) )
  {
    count = (count + stride - JEM_IDX_C(1)) / stride;
  }
}
