...
/*
  ...
  yields the rvalue mask, and the expression
    ...
    (idx_t) (SlaveNode::idof >> JEM_UIDX_C(1))
...
idx_t XConstraints::getSlaveDofs ( const IdxVector& idofs ) const
{
  ...
  for ( idx_t i = 0; i < n; i++ )
  {
    idofs[i] = (idx_t) (slaves_[i].idof >> JEM_UIDX_C(1));
  }
...
JEM_IVDEP
    ...
    for ( idx_t i = 0; i < n; i++ )
    {
      idx_t  idof = (idx_t) (slaves[i].idof >> JEM_UIDX_C(1));
...
SparseMatrix XConstraints::toMatrix () const
{
  ...
  for ( idx_t islave = 0; islave < slaveCount; islave++ )
  {
    ...
    indices[j] = (idx_t) (s.idof >> JEM_UIDX_C(1));
    values [j] = s.rvalue;
    master     = s.masterList;
...
void XConstraints::printTo ( PrintWriter& out ) const
{
  ...
  for ( idx_t islave = 0; islave < slaves_.size(); islave++ )
  {
    ...
    idx_t  idof = (idx_t) (slave.idof >> JEM_UIDX_C(1));
    idx_t  j    = 0;
...
void XConstraints::newDofCount ()
{
  ...
  if ( dofCount_ < oldCount )
  {
    ...
    {
      idx_t  idof = (idx_t) (slaves_[islave].idof >> JEM_UIDX_C(1));
...
void XConstraints::newDofOrder ( const Reordering& reord )
{
  ...
  {
    SlaveNode&    s = slaves_[islave];
    idx_t     idof  = (idx_t) (s.idof >> JEM_UIDX_C(1));

    idof            = fmap[idof];
    s.idof          = ((uidx_t) idof << JEM_UIDX_C(1)) |
                       (s.idof & JEM_UIDX_C(0x1));
...

{
  ...
  slave.idof       = ((uidx_t) idof << JEM_UIDX_C(1)) | umask;
  slave.rvalue     = rval;
  slave.masterList = 0;

  slaves_.pushBack ( slave );
...
{
  const idx_t  idof   = (idx_t) (orig.idof >> JEM_UIDX_C(1));
  const idx_t  islave = slaves_.size ();
...
void XConstraints::freeSlave_ ( idx_t islave )
{
  ...
  idof            = (idx_t) (slave.idof >> JEM_UIDX_C(1));
  slaveMap_[idof] = 0;
