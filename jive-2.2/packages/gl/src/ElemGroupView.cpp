...

{
  ...
  Color           meshColor  = mrender_->getAttribs()->getColor ();
...

{
  ...
  else
  {
    ...
    showMessageUntil ( gtx, mrender_, text,
                       Time::now() +  Time( JEM_LINT_C(8) ),
...
{
  if ( rundat_ != NIL && ! rundat_->updated )
  {
