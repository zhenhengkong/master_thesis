...

{
  ...
  for ( idx_t idom = 0; idom < domCount; idom++ )
  {
    ...
    domBlockCount = max ( domBlockCount, JEM_IDX_C(1) );
...

{
  ...
  JEM_PRECHECK ( elems != NIL && elems.size() == elemMap.size() );
...

{
  ...
  JEM_PRECHECK ( elems != NIL && elems.size() == weights.size() );

  const idx_t  elemCount = weights.size ();

  for ( idx_t ielem = 0; ielem < elemCount; ielem++ )
  {
    weights[ielem] = max ( JEM_IDX_C(1),
