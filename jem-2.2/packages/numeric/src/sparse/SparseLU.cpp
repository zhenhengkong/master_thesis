...
idx_t SparseLU::factor ( const Matrix& matrix )
{
  ...
  return factor ( matrix, mask, iperm, iperm );
}
...

{
  JEM_PRECHECK2 ( matrix.size(0) == matrix .size(1) &&
                  ...
                  matrix.size(0) == rowPerm.size()  &&
                  matrix.size(0) == colPerm.size(),
                  "Array size mismatch" );

  JEM_PRECHECK2 ( matrix.isValid(), "invalid SparseMatrix" );
  ...
  const idx_t  msize = matrix.size(1);
  ...
  trMat = transpose_ ( matrix, mask );

  for ( jcol = 0; jcol < msize; jcol++ )
  {
    jperm = upper_.colPerm[jcol];

    Array<idx_t>   irows = trMat.getColumnIndices ( jperm );
    Array<double>  mvals = trMat.getValues        ( jperm );
...
double SparseLU::getMemUsage () const
{
  const double  rsize = sizeof(double);
...
{
  const idx_t    msize   = matrix.size(0);
  const idx_t    nzCount = matrix.nonZeroCount     ();
  const idx_t    bufsz   = 2 * nzCount;

  Array<idx_t>   offsets = matrix.getRowOffsets    ();
  Array<idx_t>   indices = matrix.getColumnIndices ();
  Array<double>  values  = matrix.getValues        ();


  upper.colOffsets.resize  ( msize + 2 );
  upper.rowIndices.reserve ( bufsz );
  upper.values    .reserve ( bufsz );
  ...
  work.accu      .resize   ( msize );
  work.scratch   .resize   ( msize + 1 );
  ...
  for ( idx_t irow = 0; irow < msize; irow++ )
  {
    ...
    for ( idx_t i = offsets[jrow]; i < n; i++ )
    {
      ...
      if ( mask[indices[i]] && abs( val ) > scale )
      {
        scale = abs ( val );
      }
    }
...
SparseLU::Matrix SparseLU::transpose_
...

{
  ...
  Array<idx_t>   rowOffsets = mat.getRowOffsets    ();
  Array<idx_t>   colIndices = mat.getColumnIndices ();
  Array<double>  matValues  = mat.getValues        ();
  ...
  trValues  .resize ( colOffsets[rowCount] );

  for ( idx_t irow = 0; irow < rowCount; irow++ )
  {
    if ( ! mask[irow] )
    {
      rowIndices[colOffsets[irow]] = irow;
      trValues  [colOffsets[irow]] = 1.0;
      colOffsets[irow]++;
      continue;
    }
    ...
    for ( idx_t i = rowOffsets[irow]; i < n; i++ )
    {
      ...
      if ( mask[jcol] )
      {
        rowIndices[colOffsets[jcol]] = irow;
        trValues  [colOffsets[jcol]] = matValues[i];
        colOffsets[jcol]++;
      }
    }
  }
  ...
  return Matrix ( shape( rowCount, rowCount ),
                  colOffsets, rowIndices, trValues );
}
...

{
  ...
  ipiv = 0;
  irow = irows[ipiv];
  xpiv = scale[irow] * abs ( scratch[ipiv] );

  for ( idx_t i = 1; i < ilast; i++ )
  {
    irow = irows[i];
    xtmp = scale[irow] * abs ( scratch[i] );
