...
{
  idx_t  len = b ? JEM_IDX_C(4) : JEM_IDX_C(5);
  char*  buf = expand ( len );
...
{
  idx_t  len = (idx_t) (sizeof(i) * JEM_IDX_C(4));
  char*  buf = xalloc ( len );
...
String StringBuffer::operator [] ( const Slice&  s ) const
{
  ...
  if ( len > JEM_IDX_C(0) && stride > JEM_IDX_C(1) )
  {
    len = JEM_IDX_C(1) + (len - JEM_IDX_C(1)) / stride;
  }
...
void StringBuffer::swap ( StringBuffer& rhs )
{
  str_.swap ( rhs.str_ );
...
{
  idx_t  cap = len + size_ + size_ / JEM_IDX_C(2);
