...

{
  ...
  glColor3fv     ( RED );
  renderDisk     ( 0.3F * RADIUS, quality );

  if ( (mode_ & TRANS_XY) && (mode_ & SNAPPY) )
  {
    glLineWidth   ( 1.0F );
    glRotatef     ( (GLfloat) theta_, 0.0F, 0.0F, 1.0F );
  ...
  if ( mode_ & ROT_MODE )
  {
    ...
    {
      glColor3fv   ( RED );
      renderDisk   ( 1.2F * RADIUS, quality );
    }

    glRotatef      ( (GLfloat) theta_, 0.0F, 0.0F, 1.0F );
    glRotatef      ( (GLfloat) -zeta_, 0.0F, 1.0F, 0.0F );
    ...
    {
      EnableScope   lightScope ( GL_LIGHTING );

      glColor3fv     ( RED );
      orb_->drawOrb  ( RADIUS, Orb::PART1 );
      orb_->drawOrb  ( RADIUS, Orb::PART3 );
      orb_->drawOrb  ( RADIUS, Orb::PART4 );
      orb_->drawOrb  ( RADIUS, Orb::PART6 );

      // This seemingly useless translate solves a bug with some
      // Intel graphics drivers on Linux.

      glTranslatef   ( 0.0F, 0.0F, 0.0F );
      glColor3fv     ( YELLOW );
      orb_->drawOrb  ( RADIUS, Orb::PART2 );
      orb_->drawOrb  ( RADIUS, Orb::PART4 );
      orb_->drawOrb  ( RADIUS, Orb::PART5 );
      orb_->drawOrb  ( RADIUS, Orb::PART7 );
    }
  }
