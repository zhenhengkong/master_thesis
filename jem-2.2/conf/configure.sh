...
EOF
  ...
  if   [ "$SYS_LINT_SUFFIX" = "L" ]; then
    echo "#define JEM_LINT_IS_LONG"
  elif [ "$SYS_LINT_SUFFIX" = "LL" ]; then
    echo "#define JEM_LINT_IS_LONG_LONG"
  fi
  ...
  echo "namespace jem"
  echo "{"
  echo "  typedef $SYS_LINT lint;"
  echo "  typedef $SYS_ULINT ulint;"

  if   [ "$INDEX_TYPE" = "int" ]; then
    echo "  typedef int idx_t;"
    echo "  typedef unsigned int uidx_t;"
  else
    echo "  typedef $SYS_LINT idx_t;"
    echo "  typedef $SYS_ULINT uidx_t;"
  fi

  echo "}"
  echo ""
  echo "#define JEM_LINT_MAX   $SYS_LINT_MAX"
  echo "#define JEM_LINT_MIN   $SYS_LINT_MIN"
  echo "#define JEM_ULINT_MAX  $SYS_ULINT_MAX"

  if   [ "$INDEX_TYPE" = "int" ]; then
    echo "#define JEM_IDX_MAX  INT_MAX"
    echo "#define JEM_IDX_MIN  INT_MIN"
    echo "#define JEM_UIDX_MAX UINT_MAX"
  else
    echo "#define JEM_IDX_MAX  $SYS_LINT_MAX"
    echo "#define JEM_IDX_MIN  $SYS_LINT_MIN"
    echo "#define JEM_UIDX_MAX $SYS_ULINT_MAX"
  fi

  echo ""
  echo "#define JEM_LINT_C(i)   JEM_CONCAT(i,${SYS_LINT_SUFFIX})"
  echo "#define JEM_ULINT_C(i)  JEM_CONCAT(i,${SYS_ULINT_SUFFIX})"

  if   [ "$INDEX_TYPE" = "int" ]; then
    echo "#define JEM_IDX_C(i)  i"
    echo "#define JEM_UIDX_C(i) JEM_CONCAT(i,U)"
  else
    echo "#define JEM_IDX_C(i)  JEM_CONCAT(i,${SYS_LINT_SUFFIX})"
    echo "#define JEM_UIDX_C(i) JEM_CONCAT(i,${SYS_ULINT_SUFFIX})"
  fi
