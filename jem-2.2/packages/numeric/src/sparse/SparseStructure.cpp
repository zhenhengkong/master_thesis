...
SparseStructure::SparseStructure ( const Self& rhs ) :
...
SparseStructure& SparseStructure::operator = ( const Self& rhs )
{
  if ( this != &rhs )
  {
    shape_ = rhs.shape_;
...
void SparseStructure::swap ( SparseStructure& rhs )
{
...
bool SparseStructure::isValid () const
{
  const idx_t  nzCount = nonZeroCount ();
...
bool SparseStructure::isSorted () const
{
  const idx_t  rowCount = shape_[0];
