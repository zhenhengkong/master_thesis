...

{
  ...
  if ( coords.isContiguous() && rank < JEM_IDX_C(4) )
  {
    switch ( rank )
    {
    case 1:
...

{
  ...
  if ( boxes.isContiguous() && rank < JEM_IDX_C(4) )
  {
    const double* JEM_RESTRICT  bxp = boxes.addr ();
...

{
  ...
  if ( centers.isContiguous() &&
       boxes  .isContiguous() && rank < JEM_IDX_C(4) )
  {
