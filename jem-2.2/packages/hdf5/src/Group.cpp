...
{
  hid_t   gid = (hid_t) this->getID ();
  hid_t   oid = (hid_t) obj  .getID ();
  hid_t   pid = -1;
  herr_t  err;
...
{
  if ( findObject_( NULL, name ) )
  {
    herr_t  err = H5Ldelete ( (hid_t) oid_.get(),
                              makeCString ( name ),
                              H5P_DEFAULT );
...
bool Group::hasGroup ( const String& name ) const
{
  ...
  {
    H5I_type_t  type = H5Iget_type ( (hid_t) gid.get() );
...
{
  hid_t  gid = H5Gopen ( (hid_t) oid_.get (),
                         makeCString ( name ),
                         H5P_DEFAULT );
...
bool Group::hasDataSet ( const String& name ) const
{
  ...
  {
    H5I_type_t  type = H5Iget_type ( (hid_t) sid.get() );
...
{
  hid_t  sid = H5Dopen ( (hid_t) oid_.get (),
                         makeCString ( name ),
                         H5P_DEFAULT );
  ...
  return newInstance<DataSet> ( Handle( sid, Handle::ALLOCATED ) );
}
...

{
  ...
  if ( space )
  {
    ...
    {
      oid = H5Dcreate_anon ( (hid_t) oid_  .get   (),
                             (hid_t) type ->getID (),
                             (hid_t) space->getID (),
                             pid,
                             H5P_DEFAULT );
  ...
  else
  {
    ...
    {
      oid = H5Gcreate_anon ( (hid_t) oid_.get (),
                             H5P_DEFAULT,
                             H5P_DEFAULT );
    }
  }
  ...
  if ( oid >= 0 )
  {
    return Handle ( oid, Handle::ALLOCATED );
  }
...

{
  ...
  while ( i < len )
  {
    ...
    if ( (j - i) > 1 || name[i] != '/' )
    {
      ...
      stat = H5Oexists_by_name ( (hid_t) oid_.get (),
                                 makeCString ( name[slice(BEGIN,j)] ),
                                 H5P_DEFAULT );
  ...
  if ( oid )
  {
    hid_t  id = H5Oopen ( (hid_t) oid_.get (),
                          makeCString ( name ),
                          H5P_DEFAULT );
    ...
    *oid = Handle ( (idx_t) id, Handle::ALLOCATED );
  }
