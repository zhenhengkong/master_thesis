...
IListArray::IListArray ( idx_t n )
{
  ...
  size_    = 0;
  zlevel_  = 2;
  padding_ = (JEM_IDX_C(1) << zlevel_) / 2;
  lists_   = 0;
...
void IListArray::swap ( Self& rhs )
{
  jem::byte  buf[sizeof(Self)];

  std::memcpy ( buf,  this, sizeof(Self) );
  std::memcpy ( this, &rhs, sizeof(Self) );
  std::memcpy ( &rhs, buf,  sizeof(Self) );
}
