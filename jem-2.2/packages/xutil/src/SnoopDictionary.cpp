...
{
  if ( dict_ == NIL || ! dict_->find( value, key ) )
  {
    return false;
  }
...

{
  ...
  if ( dynamicCast<Self*>( value ) )
  {
    ...
    dict_->insert ( key, self->dict_ );

    return;
  }

  if ( e.snoop != NIL && e.snoop->dict_ != value )
  {
    e.snoop = NIL;
  }
