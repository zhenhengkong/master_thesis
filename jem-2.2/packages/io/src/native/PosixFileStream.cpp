...
void PosixFileStream::closeNoThrow ()
{
  if ( fd_ >= 0 )
  {
    int  fd = fd_;
...
int PosixFileStream::poll ( const Time& timeout )
{
  ...
  {
    msec = (int) (timeout.sec () * JEM_LINT_C(1000) +
                  timeout.nsec() / JEM_LINT_C(1000000));
  }
  ...
  {
    result = ::select ( fd_ + 1, &rfds, &wfds, &xfds, NULL );
  }
...
void PosixFileStream::writeNoThrow ( const void* buf, idx_t n )
{
  const byte*  src = (const byte*) buf;
...
int PosixFileStream::getFD () const
{
  return fd_;
}
