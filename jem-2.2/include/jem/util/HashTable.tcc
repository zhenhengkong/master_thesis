...
template <class T>
  ...
  void HashTable<T>::swap ( HashTable& rhs )
...

{
  ...
  while ( node )
  {
    if ( node->hash == hash )
    {
      eq = Traits::equalKeys ( Traits::getKey ( node->value ),
                               Traits::getKey ( item ) );
...
template <class T>
  ...
  inline idx_t HashTable<T>::capacity () const
...
template <class T>
  ...
  inline idx_t HashTable<T>::size () const
