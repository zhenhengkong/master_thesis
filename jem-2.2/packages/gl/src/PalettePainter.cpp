...
bool PalettePainter::isVisible_ () const
{
  ...
  else
  {
    ...
    t = getTransformOf ( buddy_->getParent() );
...
void PalettePainter::drawNoBuddy_ ( GfxContext& gtx )
{
  Font  f;
  Rect  r;
  ...
  palette_->render ( gtx, r, title, hAlign_, scale_, bias_ );
}
...
void PalettePainter::drawWithBuddy_ ( GfxContext& gtx )
{
  Transform  t, p;
  Vertex3d   a, b;
  Box        box;
