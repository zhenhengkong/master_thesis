...

{
  ...
  if ( evals.isContiguous() && symmat.isContiguous() )
  {
    ...
    triSolve_  ( n, evals.addr(), sd.addr(), NULL          );
  }
  else
  {
    ...
    triSolve_  ( n, dd.addr(), sd.addr(), NULL      );
...

{
  ...
  {
    triSolve_ ( n, diag.addr(), subdiag.addr(), NULL );
  }
  else
  {
