...
ItemGroupParser::Operator_*
  ItemGroupParser::Utils_::OR_OP      = NULL;
ItemGroupParser::Operator_*
  ItemGroupParser::Utils_::AND_OP     = NULL;
ItemGroupParser::Operator_*
  ItemGroupParser::Utils_::AND_NOT_OP = NULL;
ItemGroupParser::Operator_*
  ItemGroupParser::Utils_::NOT_OP     = NULL;
...

{
  ...
  if ( data.group != NIL && data.operands.size() > 0 )
  {
    updateGroup ( data );
...

{
  ...
  if ( data.groups != NIL )
  {
    ...
    if ( group != NIL && group->getItems() != data.items.get() )
    {
      group = NIL;
    }
  }
...

{
  ...
  data.operands  .pushBack ( NULL );
  data.operators .pushBack ( NULL );
...
{
  JEM_PRECHECK2 ( data.group != NIL, "NULL ItemGroup" );
  ...
  if ( data.scope != NIL && data.mustFilter )
  {
