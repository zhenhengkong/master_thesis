...
const int    SkylineSolver::REORDER      = 1 << 0;
const int    SkylineSolver::USE_THREADS  = 1 << 1;
const int    SkylineSolver::PRINT_PIVOTS = 1 << 2;
...
void SkylineSolver::setOptions ( int options )
{
  options_ = options;
  events_ |= NEW_STRUCT_;
  ...
  if ( ! (options & USE_THREADS) && (worker_ != NIL) )
  {
    worker_->cancel ();
    worker_->join   ();
...
void SkylineSolver::update_ ()
{
  ...
  if ( (options_ & USE_THREADS) && (worker_ == NIL) )
  {
...
void SkylineSolver::initSkyline_ ()
{
  ...
  {
    idx_t  iblk = d.iperm[irow] / JEM_IDX_C(4);
    idx_t  n    = rowOffsets[irow + 1];

    for ( idx_t j = rowOffsets[irow]; j < n; j++ )
    {
      idx_t  jblk = d.iperm[colIndices[j]] / JEM_IDX_C(4);
  ...
  for ( idx_t iblk = 0; iblk < bcount; iblk++ )
  {
    ...
    lowerSize += JEM_IDX_C(16) * lowerWidth;
    upperSize += JEM_IDX_C(16) * upperWidth;
...
void SkylineSolver::loadSkyline_ ()
{
  ...
  for ( idx_t irow = 0; irow < msize; irow++ )
  {
    ...
    for ( idx_t j = rowOffsets[irow]; j < n; j++ )
    {
      ...
      idx_t   jblk = d.iperm[jcol] / JEM_IDX_C(4);
      idx_t   jpos = d.iperm[jcol] % JEM_IDX_C(4);

      if ( jblk < iblk )
      {
        idx_t  k = JEM_IDX_C(16) * (jblk - d.startCols[iblk]);
        ...
        k += d.lowerOffsets[iblk] + ipos + JEM_IDX_C(4) * jpos;

        d.lowerBlocks[k] = mval;
      }
      else if ( jblk > iblk )
      {
        idx_t  k = JEM_IDX_C(16) * (iblk - d.startRows[jblk]);
        ...
        k += d.upperOffsets[jblk] + JEM_IDX_C(4) * ipos + jpos;

        d.upperBlocks[k] = mval;
      }
      else
      {
        idx_t  k = JEM_IDX_C(16) * iblk + JEM_IDX_C(4) * ipos + jpos;
  ...
  if ( bcount > 0 )
  {
    idx_t  k = JEM_IDX_C(16) * (bcount - 1);

    for ( idx_t i = msize % JEM_IDX_C(4); i < 4; i++ )
    {
