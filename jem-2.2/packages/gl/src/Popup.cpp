...

{
  ...
  case GLActions::REDRAW_FAST:
    ...
    if ( actionID_ < 0 )
    {
      gtx.cancelCallback ( hideCallback_, this );
      hide               ();
    }
    else
    {
  ...
  case GLActions::KEY_EVENT:
    ...
    if ( actionID_ >= 0 && gtx.getActionID() != actionID_ )
    {
      gtx.repostCallback ( & hideCallback_, this,
                           Time::now() + Time( 0.4 ) );
...

{
  if ( getParent() )
  {
    gtx.repostCallback ( & hideCallback_, this, tm );
  }
}
...
{
  show_              ( node.get() );
  gtx.postFastRedraw ( node );

  actionID_ = gtx.getActionID ();
}
...

{
  ...
  if ( self->getParent() )
  {
    gtx.postFastRedraw ( self->getParent() );
    self->hide         ();
  }
}
