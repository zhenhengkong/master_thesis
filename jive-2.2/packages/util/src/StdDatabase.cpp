...
class StdDBColumn : virtual public DBColumn
{
 public:
  ...
  virtual IdxVector         getNonEmptyRows () const;
  virtual void              clear           ();
...

{
  ...
  if ( n > heap_.used / JEM_IDX_C(4) )
  {
    compress_ ( heap_.used + heap_.used / JEM_IDX_C(4) );
  }
...
idx_t StdDBColumn::alloc_ ( idx_t count )
{
  ...
  if ( n > heap_.size )
  {
    ...
    if ( n > heap_.used / JEM_IDX_C(4) )
    {
      n = heap_.used + heap_.used / JEM_IDX_C(4) + count;

      compress_ ( n );
    }
    else
    {
      n = heap_.last + heap_.size / JEM_IDX_C(4) + count;
...
{
  JEM_PRECHECK2 ( items != NIL, "NULL ItemSet" );
