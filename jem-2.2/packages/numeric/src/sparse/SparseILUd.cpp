...
class SparseILUd::Node_ : public POD
{
 public:
...
double SparseILUd::getMemUsage () const
{
...

{
  ...
  double*        JEM_RESTRICT  loAccu    = NULL;
  double*        JEM_RESTRICT  upAccu    = NULL;
  idx_t*         JEM_RESTRICT  loTags    = NULL;
  idx_t*         JEM_RESTRICT  upTags    = NULL;
  idx_t*         JEM_RESTRICT  irows     = NULL;
  idx_t*         JEM_RESTRICT  jcols     = NULL;
  ...
  for ( i = 0; i < size; i++ )
  {
    upOffsets[i] = 0;
    loOffsets[i] = 0;
    rowLists [i] = JEM_IDX_C(-1);
    colLists [i] = JEM_IDX_C(-1);
    rowStart [i] = 0;
    colStart [i] = 0;
    loTags   [i] = JEM_IDX_C(-1);
    upTags   [i] = JEM_IDX_C(-1);
    loAccu   [i] = 0.0;
    upAccu   [i] = 0.0;
  }
...
JEM_IVDEP
                  ...
                  upValues_.size() + incu) > maxNnz; i++ )
    {
      ...
      flops += upValues_.size() + loValues_.size();
...
{
  const double* JEM_RESTRICT  pivots  = pivots_.addr ();
  idx_t*        JEM_RESTRICT  offsets = NULL;
  idx_t*        JEM_RESTRICT  indices = NULL;
  double*       JEM_RESTRICT  values  = NULL;
  Node_*        JEM_RESTRICT  nodes   = NULL;
  idx_t*        JEM_RESTRICT  lists   = NULL;
  idx_t*        JEM_RESTRICT  start   = NULL;
