...
inline void String::Buffer_::pushBack ( char c )
{
  if ( size_ == capacity_ )
  {
    grow_ ( size_ + 1 );
...
inline void String::Buffer_::pushBack ( char c, idx_t n )
{
...
inline idx_t String::Buffer_::size () const
{
  return size_;
}
...
inline char* String::Buffer_::addr () const
{
  return buffer_;
}
...
void String::Buffer_::grow_ ( idx_t n )
{
  char*  tmp;
  idx_t  cap;
...
inline String::Chunk_::Chunk_ ( size_t msize ) :
...
inline void String::Chunk_::dealloc ( Self* chunk )
{
...
inline char* String::Chunk_::getData ()
{
...
inline void String::Utils_::free ( String* str )
{
  if ( str->size_ > TinyString::MAXLEN && str->chunk_ )
  {
    if ( str->chunk_->refcount.decr() == 0 )
    {
      Chunk_::dealloc ( str->chunk_ );
    }
  }
}
...
String::String ( lint value )
{
  if ( value >= JEM_LINT_C(0) && value < JEM_LINT_C(10) )
  {
    size_   = 1;
    chunk_  = 0;
    str_    = buffer_;
...
String::String ( const char* rhs, idx_t len )
{
...
String::String ( const TinyString& str )
{
...
String::String ( char* first, char* last )
{
...
String::String ( const Nil& nil, idx_t len )
{
  JEM_ASSERT_NOTHROW ( len >= 0 );
...
String& String::operator = ( const String& rhs )
{
  ...
  {
    if ( chunk_ && chunk_->refcount.decr() == 0 )
    {
      Chunk_::dealloc ( chunk_ );
    }
...
String String::operator [] ( const Slice& s ) const
{
...
String String::stripWhite () const
{
  const char*  s = cstr_;
...
bool String::equals ( const char* r ) const
{
...
int String::compareToIgnoreCase ( const String& rhs ) const
{
  const char*  s = cstr_;
  const char*  r = rhs.cstr_;
...
bool String::startsWith ( const String& word ) const
{
...
bool String::endsWith ( const String& word ) const
{
  const idx_t  k = word.size_ - 1;
  const idx_t  n = size_      - 1;
...
idx_t String::find ( char needle, idx_t fromIndex ) const
{
  const char*  s = cstr_;
  const idx_t  n = size_;


  for ( idx_t i = max( JEM_IDX_C(0), fromIndex ); i < n; i++ )
  {
    if ( s[i] == needle )
    {
      return i;
...
idx_t String::rfind ( const String& needle ) const
{
  return rfind ( needle, size_ );
}


idx_t String::rfind ( char needle ) const
{
  return rfind ( needle, size_ );
}
...
idx_t String::rfind ( char needle, idx_t fromIndex ) const
{
  const char*  s = cstr_;
...
idx_t String::sizeOf ( const char* str )
{
  idx_t  len = 0;
...
void String::realloc_ ( idx_t len )
{
  if ( size_ <= TinyString::MAXLEN )
  {
    if ( len > TinyString::MAXLEN )
