...
GLsizei TextPainter::write ( char ch )
{
  ...
  glPushMatrix  ();
  glTranslatef  ( (GLfloat) xPos, (GLfloat) yPos, (GLfloat) zPos );
  rotate_       ();
...
Size2i TextPainter::writeBlock ( const String& text )
{
  Size2i  size = getSizeOf ( text );

  writeBlock ( text, size );
...

{
  ...
  glTranslatef  ( (GLfloat) xPos, (GLfloat) yPos, (GLfloat) zPos );
  rotate_       ();
  glTranslatef  ( 0.0F, (GLfloat) dy, 0.0F );
  ...
  for ( i = 0; i < len; i++ )
  {
    ...
    glTranslatef ( (GLfloat) dx, 0.0F, 0.0F );
    font.write   ( str + j, i - j );
    glTranslatef ( (GLfloat) -dx, (GLfloat) -dy, 0.0F );
  }
...
inline void TextPainter::rotate_ ()
{
