...
{
  JEM_PRECHECK ( inmod != NIL && name.size() > 0 );
...

{
  ...
  if      ( action == Actions::EXE_COMMAND )
  {
    ...
    if ( cmd == Commands::DUMP && mpx_ != NIL && fileName_.size() )
    {
      dump_ ( globdat );
