...
void AGMRES::getInfo ( const Properties& info ) const
{
  ...
  Preconditioner*  precon   =
    ...
    dynamicCast<Preconditioner*> ( precon0_ );
...
void AGMRES::configure ( const Properties& props )
{
  ...
  Precon*  precon = dynamicCast<Precon*> ( precon0_ );
...
void AGMRES::updatePrecon_ ()
{
  ...
  Preconditioner*  p = dynamicCast<Preconditioner*> ( precon0_ );
...

{
  ...
  const idx_t   m        =
    ...
    max ( JEM_IDX_C(1), min( maxVectors_, restartIter_ ) );
  ...
  if ( ! equal( precon0_->shape(), matrix_->shape() ) )
  {
